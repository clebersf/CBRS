declare 
@Origin bigint =1, -- Vertice de origem
@Destination bigint = 156, -- Vertice de destino
...
begin
	...
	BEGIN
		...
		declare @alt float = 0, @loadEdge float = 0, @Name varchar(100) -- Soma dos custos atual mais vizinho
		-- Soma dos custos atual mais vizinho
		Set @alt = (select Cost from #Dist where VertexId = @U) +
		isnull((select Size from EdgeHeuristicSize where Id = @Edge),0)

		select  @LoadEdge = Capacity, @Name = Alias from EdgeHeuristicCapacity 
		inner join Location on Location.Id = EdgeHeuristicCapacity.Id
		where EdgeHeuristicCapacity.Id = @Edge
